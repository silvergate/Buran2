...
public class CommonNode extends DocumentWrapper {
    ...
    public boolean addDomainId(DomainId domainId) {
        final Set domIdsLong = getDocument().field(FIELD_DOMAIN_IDS, OType.EMBEDDEDSET);
        boolean added = domIdsLong.add(domainId.getId());
        getDocument().field(FIELD_DOMAIN_IDS, domIdsLong, OType.EMBEDDEDSET);
        return added;
    }

    public boolean removeDomainId(DomainId domainId) {
        final Set domIdsLong = getDocument().field(FIELD_DOMAIN_IDS, OType.EMBEDDEDSET);
        boolean removed = domIdsLong.remove(domainId.getId());
        getDocument().field(FIELD_DOMAIN_IDS, domIdsLong, OType.EMBEDDEDSET);
        return removed;
    }
}
